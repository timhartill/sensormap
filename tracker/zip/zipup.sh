...
# NB 2: This version creates the config dir in the zipfile but excludes the .json confg files as this dir is volume mapped to external dir in the docker compose file (as is the logs directory)

rm tracker.zip
cd ..
zip -x *.log *.csv *.pyc *.json @ -r ./zip/tracker.zip config usecasecode logs requirements.txt README.md
